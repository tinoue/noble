...
class Noble extends events.EventEmitter {
  ...
  startScanning(serviceUuids, allowDuplicates, passiveScan, callback) {
    if (callback == null && typeof passiveScan === 'function') {
      // Old version of API without passiveScan parameter.
      callback = passiveScan;
      passiveScan = false;
    }
    ...
    const promise = new Promise((resolve, reject) => {
      const scan = function(state) {
        if (state !== 'poweredOn') {
          ...
          this._bindings.startScanning(serviceUuids, allowDuplicates, passiveScan);
        }
      };
