...
class Hci extends events.EventEmitter {
  ...
  setScanParameters(passiveScan) {
    const cmd = Buffer.alloc(11);

    if (typeof passiveScan === 'boolean') {
    ...
    // data
    debug(`Passive scan : ${this._passiveScan}`);
    cmd.writeUInt8(0x01, passiveScan ? 0 : 4); // type: 0 -> passive, 1 -> active
  ...
  processLeAdvertisingReport(count, data) {
    for (let i = 0; i < count; i++) {
      try { // Workaround for RangeError to get rssi
