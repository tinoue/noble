...
class Gap extends events.EventEmitter {
  ...
  startScanning(allowDuplicates, passiveScan) {
    this._scanState = 'starting';
    this._scanFilterDuplicates = !allowDuplicates;
    this._passiveScan = !!passiveScan;

    // Always set scan parameters before scanning
    // https://www.bluetooth.org/docman/handlers/downloaddoc.ashx?doc_id=229737
    // p106 - p107
    this._hci.setScanEnabled(false, true);
    this._hci.setScanParameters(passiveScan);
  ...
  onHciLeAdvertisingReport(status, type, address, addressType, eir, rssi) {
    ...
    if (type === 0x04 || !connectable || (discoveryCount > 1 && !hasScanResponse) || this._passiveScan || process.env.NOBLE_REPORT_ALL_HCI_EVENTS) {
